...
const MainGenealogy = () => {
  ...
  const [propsAbsolute, setPropsAbsolute] = useState(false)
  ...
  async function getSearchTree(data){
    
    try{
      const token = localStorage.getItem("token")
      let res = await axios.post("/api/searchTree",{token:token,data});
  ...
  async function getSearchTreeSubmitHandler(e){
      ...
      <section class="dashboard">
        ...
        <div class="dashboard-right">
          ...
          <div class="container-fluid">
            ...
            <div class="gen-head">
              <div class="gentree-upper">
                <div class="genup-left">
                  ...
                  <input
                    ...
                    // placeholder="Amitabh, 8965****rejc"
                    // value={
                    //   childName || childWallet
                    //     ? childName + ", " + childWallet
                    ...
                    // }
                    value={
                      childName && childWallet && childName + ',' + childWallet || !childName && childWallet || !childWallet && childName || !childName && !childWallet && userDetail?.userDetail[0]?.firstName +
                            " , " +
                            userDetail?.userDetail[0]?.address
                    }
                ...
                <div class="input-group genup-right">
                  <input
                    type="text"
                    class="form-control"
                    placeholder="Quick Search"
                    // defaultValue={(e)=>setSearch(e.target.value)}
                    onChange={(e)=>setSearch(e.target.value)}
                    value={search}
                  />
                  <img src="/magnifying-glass.png" alt="" style={{cursor:"pointer"}} onClick={()=>getSearchTreeSubmitHandler()}/>
                </div>
              </div>
              <div class="gentree-table">
                <table class="table table-dark table-striped" id="long-table">
                  <thead>
                    <tr>
                      {/* <th scope="col" id="merge"></th> */}
                      <th
                    ...
                    <tr>
                      {" "}
                      {/* <th scope="col" className="marginTop">
                        <p className="parti"> Paticular </p>
                      </th> */}
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>{userDetail?.totalLeftCount }</td>
                      <td>{userDetail?.totalRightCount}</td>
                      <td>{userDetail?.totalLeftStaking || "0"}</td>
                      <td>{userDetail?.totalRightStaking || "0"}</td>
                    </tr>
                    <tr class="ttatata">
                      <td style={{ textAlign: "left", paddingLeft: "20px" }}>
                        Matched
                      </td>
                      <td>{userDetail?.teamLeftMatch || "0"}</td>
                      <td>{userDetail?.teamRightMatch || "0"}</td>
                      <td>{userDetail?.stakingLeftMatch || "0"}</td>
                      <td>{userDetail?.stakingRightMatch || "0"}</td>
                    </tr>
                    <tr>
                      <td style={{ textAlign: "left", paddingLeft: "20px" }}>
                        Others
                      </td>
                      <td>{userDetail?.teamLeftOther || "0"}</td>
                      <td>{userDetail?.teamRightOther || "0"}</td>
                      <td>{userDetail?.stakingLeftOther || "0"}</td>
                      <td>{userDetail?.stakingLeftOther || "0"}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              {/* <!-- Tree section adds here --> */}
              <div className="genTree-section">
                {/* <Genealogy props={setNameWallet} hell={nameWallet} /> */}
                <Genealogy
                  props={{ setChildName, setChildWallet, setAdded, added, setTreeData, treeData,  setPropsAbsolute,propsAbsolute }}
                />
                <div class="gentree-bottom">
                  <span>
                    <img
                      src="/Dashboardleft.png"
                      alt=""
                      class="genleft"
                      onClick={() => closeNode()}
                    />
